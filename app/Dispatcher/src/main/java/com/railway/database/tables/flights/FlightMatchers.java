...
public class FlightMatchers {
    ...
    /* Specify one station, included in route */
    static public class MatchByFlightDirection extends Matcher {
        @Override
        public String getCondition() {
            final String format =
                    "%s IN (SELECT DISTINCT flight_number " +
                    "       FROM FLIGHT_DETAILS " +
                    "       WHERE station_id = %s)";
            return String.format(format,
                    FlightDomains.FLIGHT_NUMBER,
                    Objects.requireNonNullElse(getValue(), "?"));
        }
    }
