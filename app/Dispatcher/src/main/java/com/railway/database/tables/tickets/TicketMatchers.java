...
public class TicketMatchers {
    ...
    static public class MatchByDelayedFlight extends Matcher {
        @Override
        public String getCondition() {
            final String format =
                    "%s BETWEEN " +
                    "TO_DATE(%s, 'DD-MM-YYYY HH24:MI:SS') AND \n" +
                    "TO_DATE(%s, 'DD-MM-YYYY HH24:MI:SS') + %s / 24";
            return String.format(format,
                    TicketDomains.OPERATION_DATE,
                    FlightDomains.FLIGHT_DATE,
                    FlightDomains.FLIGHT_DATE,
                    DelayDomains.DELAY_DURATION);
        }
    }
